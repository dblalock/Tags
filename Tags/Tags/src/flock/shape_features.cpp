...
//	PRINT_VAR(ar::min(scores, seqLen));
//	PRINT_VAR(ar::max(scores, seqLen));
	
//	max_inplace(scores, seqLen, 0); // ensure nonnegative from floating pt err
	
	auto downsampled = ar::resample(scores, seqLen, 80);
	auto nanIdxs = ar::where_nan(scores, seqLen);
	auto negIdxs = ar::where_negative(scores, seqLen);
	PRINT_VAR(ar::to_string(nanIdxs));
	PRINT_VAR(ar::to_string(negIdxs));
	PRINT_VAR(ar::min(scores, seqLen));
	PRINT_VAR(ar::max(scores, seqLen));
	plot_array(downsampled.get(), 80, -1, .5, "scores");
